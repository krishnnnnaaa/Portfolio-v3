...
export class Status{
    ...
    async saveDocument({id, title, time, tool, workTool, toggleTool}: {id:string, title:string, time:string, tool:string, workTool:string, toggleTool: boolean}){
        try {
            return await this.databases.updateDocument(process.env.NEXT_PUBLIC_APPWRITE_DATABASE_ID || '', process.env.NEXT_PUBLIC_APPWRITE_STATUS_COLLECTION_ID || '', process.env.NEXT_PUBLIC_APPWRITE_STATUS_DOCUMENT_ID || '', {id, title, time, tool, workTool, toggleTool})
        } catch (error) {
