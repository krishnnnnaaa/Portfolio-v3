...
export default function Manage() {
  ...
  const { id, time, title, tool, trackInfo, workTool, toggleTool, toggleIcon } = useStatus();
  ...
  const handleStatusEvent = () => {
    ...
    setisSubmmited(true);
    status.saveDocument({ id, time, title, tool, workTool , toggleTool, toggleIcon});
      ...
      <div className="items-center flex w-full  md:w-[70%] flex-col px-8">
        <div className="flex justify-between space-y-8 md:space-x-0 md:flex-row flex-col w-full items-center md:items-end">
          <div>
            ...
            <Status id={id} time={time} title={title} tool={tool} key={id} workTool={workTool} toggleTool={toggleTool} toggleIcon={toggleIcon} />
          </div>
